...
def resolve_output_dir(cli_value: Optional[str] = None) -> Path:
    raw = cli_value or os.environ.get("OPENAI_REGISTER_OUTPUT_DIR") or str(OUT_DIR)
...
def fetch_sentinel_token(*, flow: str, did: Optional[str], proxies: Any = None) -> Optional[str]:
    try:
...
def main() -> None:
    ...
    parser.add_argument("--count", type=int, default=0, help="Number of attempts to run, 0 means unlimited")
    parser.add_argument("--output-dir", default=None, help="Directory used to save generated tokens and account files")
    args = parser.parse_args()

    sleep_min = max(1, args.sleep_min)
    sleep_max = max(sleep_min, args.sleep_max)
    target_count = max(0, args.count)
    if args.once and target_count == 0:
        target_count = 1
    ...
    while True:
        ...
        if args.once or (target_count and count >= target_count):
            break
